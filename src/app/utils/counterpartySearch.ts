...
function looksLikeFio(text: string): boolean {
  // Heuristics for RU FIO:
  // - contains Cyrillic letters
  // - 1-3 tokens
  // - tokens look like Name/Surname OR initials
  ...
  // For Latin FIO (EN): require 2-3 tokens and at least one TitleCase token (to avoid treating nicknames like "fast trader" as fio)
  if (hasLat && !hasCyr) {
    if (tokens.length < 2 || tokens.length > 3) return false;
    const hasTitle = tokens.some((t) => /^[A-Z][a-z]+$/.test(t));
    const allOk = tokens.every((t) => /^[A-Za-z]+\.?$/.test(t) || /^[A-Z]\.?$/.test(t) || /^[A-Z][a-z]+$/.test(t));
    if (!hasTitle || !allOk) return false;
  }
  // Allow "Иванов", "Иванов Иван", "Иванов И", "Иван И.", "Иванов Иван Иванович"
  const tokenOk = (t: string) => {
    // Initial like "И" or "И." or "I" / "I."
  ...
  // If it's a single token with digits/specials, not fio.
  if (tokens.length === 1 && /[0-9_@#]/.test(tokens[0])) return false;

  // If it has at least 2 tokens OR one token and starts with capital -> treat as fio.
  if (tokens.length >= 2) return true;
  const one = tokens[0];
  return /^[A-ZА-ЯЁ][a-zа-яё]+/.test(one);
}
